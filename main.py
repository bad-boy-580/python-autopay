...
TOKEN = "5222584817:AAFHAIiYgPm2jCCbTeRTp31j3xF71UH-qxE"
refr = [0.1]
dci=[0.1]
wmin=[1.0]
wmax=[100]
...
def bas(update, context):
    ...
    if c=="🎁 Daily Checkin":
        ...
        reply_markup = ReplyKeyboardMarkup(dash_key,resize_keyboard=True)
        user = str(update.effective_user.id)
        msg = 'https://t.me/Python_AutoPay_Bot?start={}'.format(data['id'][user])
...
def ham(update, context):
    ...
    try:
        ...
        orderID+=number
        paytmParams["subwalletGuid"]      = "5aE2svwq0HcAnga3PzruCq8IyM2ZBeop"
